package com.learn.collectionlist;
...
public class ArrayListExample {
	 ...
	 * The underlying DS is resize able Array or Grow able Array. We can insert Heterogeneous objects as well.
	 * NOTE: All the collections can store Heterogeneous objects can be stored except TREE SET and TREE MAP.
	 * ArraList and vector implements RandomAccess, Serialize able and Clone able Interfaces
	 * Synchronized-> No
	 * Thread safe-> NO
	 * Default capacity-10
	...
	public void arraylistexample(){
		...
		for(int size=0; size<anotherList.size();size++){
			System.out.println("Using for loop element at "+size+"th position " +anotherList.get(size));
		}

		//fetch using advanced for loop
		for (String string : anotherList) {
			System.out.println("List elements using for eachloop "+string);
		}
